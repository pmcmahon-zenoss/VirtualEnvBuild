...
patch_packages=$(for patch in $(cd $BUILDDIR/inst/externallibs; ls *patch*); do echo ${patch}|perl -ne 'if (m/(.*)-r?\d/g){print "$1\n"}'; done|sort|uniq)
...
do
   ...
   mkdir patch
   cp $BUILDDIR/inst/externallibs/$package* patch
   cd patch
